...
async def run_session(test_mode: bool = False) -> None:
    ...
    # 6. Send game-over summary to all agents
    await send_game_over_summary(session.cumulative_scores, session.rounds, selected_agents)
    await asyncio.sleep(3)  # give agents time to receive it
